#include <iostream>

// including from Standard Template Library for used without the need to create the 'wheel' again.
#include <algorithm>  // std::transform, std::remove_if, std::for_each
#include <memory>     // std::unique_ptr
#include <numeric>    // std::accumulate

using namespace std;


int calc_sum(int repeats, const unique_ptr<int[]>& numbers);
int find_min(int repeats, const unique_ptr<int[]>& numbers);
int find_max(int repeats, const unique_ptr<int[]>& numbers);

int main() {
	// cout << "Hello World 1!" << "\n";

	int repeats = 0;
	cout << "How many numbers? " ; // Type a number and press enter
	cin >> repeats ;

	// int nums[] = {}; // tried to use make an array to keep the numbers in  
	// Use of point to array instead
	unique_ptr<int[]> numbers(new int[repeats]);
	
	for (int x = 0; x < repeats; x++) {
		// had x = 1 be in failed attempt
		// cout << "Number " << nums << ": ";
	// looping with x , tried to use 'nums' as numbers that is going to be used for calculations
	cout << "Number " << x + 1 << ": ";
	// x + 1 is because zero-based vs starting to count at 1
		// cin >> nums ;
    // failed, need to use the pointer to the number instead
	cin >> numbers[x];

	    // cout << "\n";
	// not needed to have new line here, from old failed try
	}
	// first test to make sure it was able to calculate
    // cout << nums * 2;

	
   // Calculate the sum, min, and max using reduce functionality by using the
   // accumulate function.

   auto numbers_end = numbers.get() + repeats;

   // Note the default behavior of accumulate is to add each element. So
   // we can find the sum of the numbers in the array like this:
   auto sum = accumulate(numbers.get(), numbers_end, 0);
//    This function was made able because we included 'accumulate'

   // Alternatively, we could use a lambda function to find the sum
   auto alternative_sum = accumulate(numbers.get(), numbers_end, 0,
      [](auto sum, auto number) { return sum + number; });
	  //    This function was made able because we included 'accumulate'


   // Calculate the average using the sum. Note we need to explicitly
   // cast one of the variables in the calculation to a floating point number
   // or we will end up doing integer division instead of floating point
   // division
   auto average = sum / (float)repeats;

   // Calculate the min using accumulate with a lambda function
   //    This function was made able because we included 'accumulate'
   auto min = accumulate(numbers.get(), numbers_end, numbers[0],
      [](auto min, auto number) {
      return (number < min) ? number : min;
      });

   // Calculate the max using accumulate with a lambda function
   //    This function was made able because we included 'accumulate'
   auto max = accumulate(numbers.get(), numbers_end, numbers[0],
      [](auto max, auto number) {
      return (number > max) ? number : max;
      });

   cout << endl;
   cout << "Sum: " << sum << endl;
   cout << "Average: " << average << endl;
   cout << "Minimum: " << min << endl;
   cout << "Maximum: " << max << endl;


   // Square all numbers in the list using map (transform)
   numbers_end = numbers.get() + repeats;
   //    This function was made able because we included 'transform'
   numbers_end = transform(numbers.get(), numbers_end, numbers.get(),
      [](auto x) { return x * x; });
   cout << "Square of each number:";
   for_each(numbers.get(), numbers_end, [](auto x) { cout << " " << x; });
   cout << endl;

   // Find all numbers less than 1000 using filter (remove_if)
    //    This function was made able because we included 'remove_if'
   numbers_end = remove_if(numbers.get(), numbers_end,
      [](auto x) {return x >= 1000; });
   cout << "Numbers less than 1000:";
   for_each(numbers.get(), numbers_end, [](auto x) { cout << " " << x ; });
   cout << endl;

   // Of the numbers less than 1000, find all odd numbers 
   //    This function was made able because we included 'remove_if'
   numbers_end = remove_if(numbers.get(), numbers_end,
      [](auto x) { return (x % 2) == 0; });
   cout << "Odd numbers:";
   for (int i = 0; i < numbers_end - numbers.get(); i++) {
      cout << " " << numbers[i];
   }
   cout << endl;
	  // Get 5 more numbers
   repeats = 5;
   unique_ptr<int[]> numbers2(new int[repeats]);
   cout << endl;
   cout << "Enter 5 more numbers: " << endl;
   for (int i = 0; i < repeats; i++) {
      cout << "Number " << i + 1 << ": ";
      cin >> numbers2[i];
   }

   // what if you accidentally pass in repeats+1 instead of repeats?
// The 'Function prototypes' made this possible
   sum = calc_sum(repeats, numbers2);
   average = (float)sum / repeats;
   min = find_min(repeats, numbers2);
   max = find_max(repeats, numbers2);

   cout << endl;
   cout << "Sum: " << sum << endl;
   cout << "Average: " << average << endl;
   cout << "Minimum: " << min << endl;
   cout << "Maximum: " << max << endl;
   return 0;
}

// The 'Function prototypes' made this possible
int calc_sum(int count, const unique_ptr<int[]>& numbers)
{
   int sum = 0;

   // what if you mistakenly use <= instead of < here?
   for (int i = 0; i < count; i++) {  
      sum += numbers[i];
   }
   return sum;
}
// The 'Function prototypes' made this possible
int find_min(int count, const unique_ptr<int[]>& numbers)
{
   int min = numbers[0];
   for (int i = 1; i < count; i++) {
      min = (numbers[i] < min) ? numbers[i] : min;
   }
   return min;
}

// The 'Function prototypes' made this possible
int find_max(int count, const unique_ptr<int[]>& numbers)
{
   int max = numbers[0];
   for (int i = 1; i < count; i++) {
      max = (numbers[i] > max) ? numbers[i] : max;
   }
   return max;
}
